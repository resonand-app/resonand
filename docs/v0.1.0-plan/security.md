...
- [x] **SEC-1** · **A ceiling on a request body, enforced before anything reads it.** Every other
      ...
      `RESONAND_MAX_UPLOAD_BYTES`, the bytes were already on disk — the same disk as the database
      ...
      See [`api/limits.py`](../../backend/resonand/api/limits.py). 🧪
...
- [x] **SEC-2** · **The sign-in path stopped answering through the clock.** Every failure returns
      ...
      real to verify against. See [`api/security.py`](../../backend/resonand/api/security.py). 🧪
...
- [x] **SEC-3** · **What the sign-in throttle counts against.** Counting per address alone was
      ...
      See [`api/rate_limit.py`](../../backend/resonand/api/rate_limit.py). 🧪

- [x] **SEC-4** · **The headers a browser needs in order to defend the page.** Resonand serves its
      ...
      [`api/headers.py`](../../backend/resonand/api/headers.py). 🧪
