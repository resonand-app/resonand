...
- [x] **INT-1** · **View I · Trash**: deleted audios and libraries with the time they have left,
...
- [x] **INT-2** · Scheduled trash purge at the configured retention (30 days by default, per
      instance), as a recurring job, also deleting the files from `storage/`. ⇢ INT-1, JOB-1 🧪

- [x] **INT-3** · **View H · Administration**, a section inside `UI-20`'s Settings shown only when
...
- [x] **INT-5** · Security pass: signed URLs, size limits, login rate limiting, and verification
...
- [x] **INT-3b1** · Deleting an account counts only what it **owns**, so an account that uploaded
      ...
      ⇢ INT-3b, API-20 🧪
      *Four counts now, assembled into one sentence that lists only what it found — a count of
      nothing does not earn a clause. The 500 was reproduced before it was fixed: with the two new
      counts taken out, the delete answers `sqlite3.IntegrityError: FOREIGN KEY constraint failed`,
      which is what an administrator was reading as "Unexpected error".*
