...
function show() {
  ...
  return render(
    <QueryClientProvider client={client}>
      <MemoryRouter initialEntries={[toLibrarySettings(RECORDINGS)]}>
        <Routes>
...
function categories() {
  return archive.categories[RECORDINGS] ?? [];
}
...
describe('the tree', () => {
  ...
  it('renames one on blur, like every other name in the product', async () => {
    show();
    // Exactly "Interviews": the move, delete and parent controls all carry the name too.
    await userEvent.click(await screen.findByRole('button', { name: 'Interviews' }));
    const field = screen.getByDisplayValue('Interviews');
  ...
  it('moves one under another, and refuses to move it under itself', async () => {
    show();
    const parent = await screen.findByRole('combobox', { name: /What Fieldwork sits under/i });
    ...
    expect(screen.queryByRole('option', { name: 'Fieldwork' })).not.toBeInTheDocument();
    await userEvent.click(screen.getByRole('option', { name: 'Interviews' }));
    await waitFor(() => {
      expect(categories().find((one) => one.name === 'Fieldwork')?.parent_id).toBe(1);
    });
  });

  it('reorders siblings by sending the whole sibling list in its new order', async () => {
    show();
    await userEvent.click(await screen.findByRole('button', { name: /Move Fieldwork up/i }));
    await waitFor(() => {
      expect(categories().find((one) => one.name === 'Fieldwork')?.position).toBe(0);
    });
    expect(categories().find((one) => one.name === 'Interviews')?.position).toBe(1);
  });
  ...
  it('states in numbers what a deletion costs, and that it costs no recordings', async () => {
    archive.recordings = archive.recordings.map((one, index) =>
      index === 0 ? { ...one, category_id: 1, library_uuid: RECORDINGS } : one,
    );
    show();
    await userEvent.click(await screen.findByRole('button', { name: /Delete Interviews/i }));
  ...
  it('deletes the category and leaves the recordings where they are', async () => {
    ...
    await waitFor(() => {
      expect(categories().map((one) => one.name)).not.toContain('Interviews');
    });
