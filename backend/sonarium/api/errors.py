...
* Anything the caller cannot read at level 10 is a **404, not a 403** (``DEC-14``). That is decided
  in the access layer, which raises :class:`~sonarium.core.errors.NotFoundError`; this module only
  has to not undo it.
...
_STATUS_BY_ERROR: tuple[tuple[type[SonariumError], int, str], ...] = (
...
def _status_and_title(error: SonariumError) -> tuple[int, str]:
...
def install_error_handlers(app: FastAPI) -> None:
    ...
    @app.exception_handler(SonariumError)
    async def _handle_sonarium_error(request: Request, exc: Exception) -> JSONResponse:
        error = exc if isinstance(exc, SonariumError) else SonariumError(str(exc))
