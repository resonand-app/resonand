...
Every setting is read from the environment with the ``RESONAND_`` prefix. Nothing is read from a
...
class Settings(BaseSettings):
    ...
    """Overrides ``<data_dir>/resonand.db``. Set to ``:memory:`` only in tests."""
    ...
    base_path: str = ""
    """Mount point behind a reverse proxy: empty for a subdomain, ``/resonand`` for a subpath."""
    ...
    session_cookie_name: str = "resonand_session"
    ...
    * ``false`` -- never marked. For an instance that is plain HTTP on purpose and would rather
    ...
    def resolved_database_path(self) -> Path:
        ...
        return self.data_dir / "resonand.db"
    ...
    def _an_empty_secret_is_no_secret(cls, value: object) -> object:
        """``RESONAND_SECRET_KEY=`` means the operator has not set one.
    ...
    def configuration_problems(self, *, require_secret: bool = True) -> ConfigurationReport:
        ...
        if require_secret and self.secret_key is None:
            problems.append(
                "RESONAND_SECRET_KEY is not set. It signs the short-lived tokens that let the "
            ...
            problems.append(
                "RESONAND_SECRET_KEY is shorter than 32 characters. Generate one with: "
                "openssl rand -hex 32"
            )

        if not self.is_memory_database:
            problems.extend(_directory_problems("RESONAND_DATA_DIR", self.data_dir))
            database_parent = self.resolved_database_path.parent
            if database_parent != self.data_dir:
                problems.extend(_directory_problems("RESONAND_DATABASE_PATH", database_parent))
            storage = self.resolved_storage_dir
            if storage.parent != self.data_dir:
                problems.extend(_directory_problems("RESONAND_STORAGE_DIR", storage))

        if self.transcription_base_url is None:
            advisories.append(
                "RESONAND_TRANSCRIPTION_BASE_URL is not set, so no recording can be "
