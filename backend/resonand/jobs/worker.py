...
which is what ``resonand work`` uses.
"""
...
from resonand.core import changes
from resonand.core.changes import Change
from resonand.core.errors import ResonandError
from resonand.db.models import Audio
from resonand.jobs import queue, retention
from resonand.jobs.handlers import HANDLERS, Context
...
class Worker:
    ...
    def start(self) -> None:
        ...
        for index in range(self._concurrency):
            thread = threading.Thread(
                target=self._loop, name=f"resonand-worker-{index}", daemon=True
            )
            thread.start()
    ...
    def wait(self) -> None:
        """Block until the worker is stopped. What a foreground ``resonand work`` does."""
