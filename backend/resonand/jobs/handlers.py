...
from resonand.core.changes import Changes
from resonand.core.config import Settings
from resonand.core.errors import InvalidRequestError, NotFoundError
from resonand.db import search_index, transcripts
from resonand.db.audio import remove_recording
...
from resonand.jobs import retention
from resonand.jobs.queue import (
    KIND_TRANSCODE,
    KIND_TRANSCRIBE,
    KIND_WAVEFORM,
    Work,
...
from resonand.media import recorded_at as recorded_at_module
from resonand.media import storage, transcode, waveform
...
from resonand.transcription.contract import TranscriptionProvider, TranscriptionRequest
...
class Context:
    ...
    Optional because a handler must run without one: ``resonand work`` on its own and every test
...
def handle_probe(work: Work, context: Context) -> None:
    ...
    with context.database.write_session() as session:
        ...
        if derived is not None and audio.recorded_at is None:
            ...
            audio.recorded_at_precision = derived.precision
        session.flush()
...
def handle_purge(work: Work, context: Context) -> None:
    ...
    between the two leaves an orphaned file, which ``resonand fsck`` reports and an operator
...
def _locate(work: Work, context: Context) -> tuple[str, Path]:
    ...
    if not path.exists():
        raise NotFoundError(
            f"The original file for {audio_uuid} is missing from storage. Run resonand fsck."
        )
