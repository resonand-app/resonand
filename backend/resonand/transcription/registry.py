...
from resonand.transcription.contract import TranscriptionProvider
from resonand.transcription.metering import UsageSink
from resonand.transcription.openai_compatible import PROVIDER_NAME, OpenAiCompatibleProvider
...
def _openai_compatible(settings: Settings, usage: UsageSink | None) -> TranscriptionProvider:
    if not settings.transcription_base_url:
        raise ConfigurationError(
            "RESONAND_TRANSCRIPTION_BASE_URL is not set, so nothing can be transcribed. Point it "
