...
from resonand.cli import integrity
from resonand.cli.backup import backup_database, storage_note, verify_backup
from resonand.core.config import MINIMUM_SECRET_LENGTH, Settings, get_settings
from resonand.core.errors import ResonandError
from resonand.core.formats import is_accepted
from resonand.db import search_index, users
...
from resonand.jobs.queue import enqueue, enqueue_transcription
from resonand.jobs.worker import Worker, drain
from resonand.media import storage
from resonand.media.probe import probe as probe_media
...
@app.callback()
def main() -> None:
    """Keeps every command explicitly named, so ``resonand import`` never becomes the default."""


@app.command()
def version() -> None:
    """Print the version and exit."""
    typer.echo(f"resonand {__version__}")
...
def openapi(
    ...
    if check:
        ...
        if current != document:
            typer.echo(
                f"{output} is not what the API publishes. Run `resonand openapi` and commit it.",
                err=True,
            )
...
def import_files(
    ...
    **Re-import is idempotent.** A file next to a Resonand sidecar carrying a ``uuid`` that is
...
def check_transcription(
    ...
    provider = build_provider(settings)
    try:
        with TemporaryDirectory(prefix="resonand-probe-") as workspace:
            if audio is None:
...
def create_admin(
    ...
    """
    from resonand.api.security import hash_password  # noqa: PLC0415 -- CLI-only dependency
    ...
    # The personal library comes with the account, and its uuid is what `resonand import` takes.
...
def _snapshot_settings() -> Settings:
    ...
    Explicit rather than ``get_settings()``: a ``RESONAND_BASE_PATH`` in the shell that generated
...
def _sidecar_beside(path: Path) -> Path | None:
    ...
    The directory's own ``resonand.json`` is only taken when this is the one recording in it,
...
def _destination_library(
    ...
    if payload is not None:
        ...
        if named is not None:
            return named
    if library_uuid is None:
        raise ResonandError("there is no library to import into: pass --library")
    library = session.query(Library).filter(Library.uuid == library_uuid).one_or_none()
    if library is None:
        raise ResonandError(f"There is no library with uuid {library_uuid}.")
    return library
