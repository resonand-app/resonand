...
against ``RESONAND_MAX_UPLOAD_BYTES``, the bytes are on disk. And the body is parsed before the
...
from resonand.core.config import MEGABYTE
...
class RequestSizeLimit:
    ...
    :class:`~resonand.api.logging.RequestCorrelationMiddleware` gives: the streaming endpoint
    ...
    body = json.dumps(
        {
            ...
            "detail": (
                ...
                "its own; raise RESONAND_MAX_UPLOAD_BYTES if that is the one you have hit."
            ),
            "status": 413,
        }
    ).encode()
