...
from resonand.acl.query import require_audio, require_library
...
from resonand.api.presenters import audio_detail
from resonand.api.schemas import AudioDetail
from resonand.api.stream_tokens import issue, redeem
...
from resonand.media import ranges, storage, waveform
...
def playback_token(
    ...
    if secret is None:
        raise InvalidRequestError(
            "This instance has no RESONAND_SECRET_KEY, so playback links cannot be signed."
        )
    ...
    while True:
        ...
        if written > limit:
            raise InvalidRequestError(
                ...
                f"({limit // (1024 * 1024)} MB). Raise RESONAND_MAX_UPLOAD_BYTES if that is not "
                "what you meant."
            )
        yield chunk
