...
Everything here goes through :mod:`resonand.acl.query`; nothing queries ``audio`` directly.
...
from resonand.acl.query import audio_acl, audio_select, require_audio, require_library
...
from resonand.core.text import clean_title
from resonand.core.time import PRECISION_SECOND, is_wall_clock, now_instant
from resonand.db import search_index
from resonand.db.models import Audio, Library, Share, User
from resonand.db.search import Filters, SortDirection, SortField, apply_filters
from resonand.db.tags import set_audio_tags
...
def library_audio(
    ...
    *,
    ...
    Filtering goes through :func:`resonand.db.search.apply_filters`, the same function search
...
def remove_recording(session: Session, audio: Audio) -> str:
    ...
    **It resolves nothing.** Both callers have already decided they may do this -- the retention
    ...
    points at, which ``resonand fsck`` reports as orphans. The other way round leaves rows
...
def share_audio(
    ...
    **The grant is on the recording and reaches nothing else.** It does not make the library
    ...
    :func:`resonand.acl.query.audio_acl` rather than here.
...
def _require_category_in_library(session: Session, audio: Audio, category_id: int) -> None:
    ...
    from resonand.db.models import Category  # noqa: PLC0415 -- kept local to avoid a cycle
