...
through :mod:`resonand.acl.query` rather than by checking anything itself. The levels are:
...
from resonand.acl.query import library_acl, library_select, require_library
...
def purge_library(session: Session, user_id: int, library_uuid: str) -> list[str]:
    ...
    the reason :func:`resonand.db.audio.remove_recording` gives.
    ...
    **Only from the trash**, and a library that is not in it answers as though it were not there,
    for the same reason :func:`resonand.db.audio.purge_audio` does.
    """
