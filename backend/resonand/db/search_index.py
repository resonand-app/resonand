...
``resonand reindex`` has one place to call.
...
:func:`resonand.db.engine.write_session`, so that a metadata change and its index update land
...
def rebuild_audio_index(session: Session) -> int:
    ...
    ``resonand reindex`` is what got indexed and not what should have been.
    """
...
def rebuild_all(session: Session) -> int:
    """Rebuild both indexes -- what ``resonand reindex`` calls. Returns the recordings covered."""
